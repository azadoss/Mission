class AudioProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    const input = inputs[0][0] || new Float32Array(0); // Handle empty input
  ...
  sendBuffer() {
    if (this.bufferIndex === 0 || this.isProcessing) return;
    ...
    const level = Math.min(rms * 100 * 5, 100); // Scale to 0-100 range
